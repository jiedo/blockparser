...
struct DumpBlock:public Callback
{
    ...
    virtual const char                   *name() const         { return "dumpBlock"; }
    ...
    {
        v.push_back("blockinfo");
        v.push_back("blockshow");
        v.push_back("showblock");
        v.push_back("blockdetails");
    }
    ...
    )
    {
        ...
        for(size_t i=1; i<args.size(); ++i) {
            rootHashes.push_back(strtoull(args[i].c_str(), &args_end, 10));
        }

        if(0<rootHashes.size()) {
            info("dumping %d blocks\n", (int)rootHashes.size());
        } else {
            warning("no Block id specified, using Block 1");
            rootHashes.push_back(1);
        }
    ...
    virtual void startBlock(const Block *b, uint64_t) {
        ...
        if (lastBlkVersion != version) {
            // info("\nnew block(%lu) version: %d\n", currBlock, version);
            lastBlkVersion = version;
        }

        if(nbDumped == rootHashes.size()) {
            info("finished dump\n");
            exit(0);
        }
        needDump = (rootHashes.end() != std::find(rootHashes.begin(), rootHashes.end(), currBlock));
        if (needDump) {
            ++nbDumped;
            ...
            showHex(b->hash);
            printf("\nblk = ");
            showHex(b->chunk->getData(), b->chunk->getSize(), false);
            printf(" [%lu]\n", b->chunk->getSize());
        }
    }
