...
// g++ -m64 -O6 -DWANT_DENSE -I. -lcrypto bruteforce_brain_address.cpp util.cpp sha256.cpp rmd160.cpp opcodes.cpp -o bbb
...
int test_phrase(const char *passphrase, EC_KEY *pkey){
...
int main()
{
  ...
  while(1) {
    ...
    test_phrase((const char *)getline_buf, pkey);
  }
  free(getline_buf);
  fclose(fp_phrase);
